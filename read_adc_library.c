...
gcc -shared -o read_adc.so -fPIC read_adc.c
...
int read_adc(int repeat_micros, int samples)
{
   int i, wid, offset;
   char buf[2];
   gpioPulse_t final[2];
   ...
   REPEAT_MICROS = repeat_micros;
   SAMPLES = samples;

   printf("REPEAT_MICROS %d SAMPLES %d\n", REPEAT_MICROS, SAMPLES);
   ...
   */
   ...
   while (sample<SAMPLES)
   {
      ...
      //usleep(REPEAT_MICROS);
   }

   end = time_time();

   printf("DATA_OUTPUT_STOP\n");
